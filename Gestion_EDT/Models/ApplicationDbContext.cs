...
namespace Gestion_EDT.Data
{
    public class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // ── Contraintes métier existantes ───────────────────────
            modelBuilder.Entity<Seance>()
                .ToTable(t => t.HasCheckConstraint(
                    "CHK_Seance_Heures", "heure_fin > heure_debut"));

            modelBuilder.Entity<Seance>()
                .ToTable(t => t.HasCheckConstraint(
                    "CHK_Seance_Semaine", "semaine BETWEEN 1 AND 52"));

            modelBuilder.Entity<Groupe>()
                .ToTable(t => t.HasCheckConstraint(
                    "CHK_Groupe_NbEtudiant", "nb_etudiant > 0"));

            modelBuilder.Entity<Salle>()
                .ToTable(t => t.HasCheckConstraint(
                    "CHK_Salle_Capacite", "capacite > 0"));
            ...
            })
            {
                foreach (var n in niveaux)
                    parcoursData.Add(new Parcours { Id = pid++, nom_parcours = n, CycleId = cycleId });
            }
            ...
            // Voir Program.cs → création automatique au premier démarrage.
        }
    }
}
